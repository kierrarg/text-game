...
#                        print(stat.inventory[1])
    ...
    def centriMeeting():
        while True:
            ...
            while stat.answer == "0":
                ...
                print(stat.name[2], "'Welcome all, please take a seat")
                if Stats.diary == True:
                    print(stat.name[0], "Centri this is Arien, Astra's daughter'")
                    print("Her eyes shoot wide open and a bead of sweat begins to roll down her face")
                    print(stat.name[2], "'Ah Arien, well the years have flown by haven't they? It truly is a pleasure to see you again'\n[1] 'Yes they have, it's a pleasure to see you as well'\n[2] 'You look a little nervous, are you okay?'")
                    Stats.options()
                    stat.answer = input(">>")
                    while stat.answer == "1":
                        print(stat.name[2], "'Well I believe it is time to begin discussing our plans'")
                        ...
                        return None
                    if stat.answer == "2":
                        print(stat.name[0], "'She's right Centri, are you all right?'")
                        print(stat.name[2], "'Yes I am fine, I believe it is time to begin discussing our plans'")
                        print("You notice Centri fidgetting with her fingers and shaking as\nshe begins to extend her arm gesturing to the table in the center of the room.")
                        return None
                    else:
                        Stats.statsFuncs()
                    ...
                    print(stat.name[0], "'Centri this is my associate'\n")
                    print("Centri smiles at you and nods her head, her presence is warm and kind")
                    print(stat.name[2], "'Well it is a pleasure to meet you, I hope you all had a safe trip'\n[1] 'Yes I managed to get some much needed rest'")
                    Stats.options()
                    stat.answer = input(">>")
                    while stat.answer == "1":
                        print(stat.name[2], "'Well with that all said and done, I believe it is time to discuss our plans'")
        ...
        while True:
            ...
            while stat.answer == "0":
                ...
                while stat.answer == "1" or "2":
                    print(stat.name[4], "'Later tonight we will board the ships and head out the Gelni, it is imperative that the weapons are prepared and boarded onto the plane'")
                    if Stats.diary == True:
                        ...
                        stat.answer = input(">>")
                        if stat.answer == "1":
                            print(stat.name[3], "'Yes well he's very involved in the Videan diplomatic affairs,\nthe success of this war is very important to him'\nRise is also acting strange, they are clearly keeping something hidden.")
                            print(stat.name[3], "'Arien would you like to help me fuel the ships, or help Evip pack up the weapons'\n[1] Help Rise\n[2] Help Evip") 
                            stat.answer = input(">>")
                            if stat.answer == "1":
                                Npc.rise()
                                return None
    ...
    def rise():
        while True:
            MiniGame.fuelShip()
            print(stat.name[3], "'Look at that, the ships are all fueled up, thanks for your help Arien'")
            if Stats.diary == True:
                print("Rise looks around to see if anyone is listening")
                print(stat.name[3], "'Between the two of us, though I am loyal to the nation of Vide. I grew up in the same nation as you'\nshe leans over and whispers into your ears")
                print(stat.name[3], "'I have reason to believe Centri is behind the betrayal of our home all those years ago.\nI do not think this war will play out how you guys are expecting it to.\nYou cannot trust the Videans'")
    ...
    def evip():
        while True:
            stat.answer = "0"
            while stat.answer == "0":
                print(stat.name[4], "'I don't know about you but I find loading the weapons onto the ship incredibly boring\nWhat do you say we make this a little more entertaining and we try doing some riddles to make this a little more interesting?'\n[1] 'Sounds like a great idea'\n[2] 'Would it be alright if we just loaded the ship in silence?'")
                Stats.options()
                stat.answer = input(">>")
                if stat.answer == "1":
                    MiniGame.riddlesPrint()
                    print(stat.name[4], "'Now that was a fun way to pass the time'")
                    return None
                    
                elif stat.answer == "2":
                    print(stat.name[4], "'That's fair, well you can carry those smaller bags over there and I'll load up these larger ones'")
    ...
    def gungarMeet():
        while True:
            print(stat.name[0], "'If possible, Arien has a list of questions she would like to ask\n")
            if Stats.diary == True:
                ...
                print("Centri chokes for a minute\n", stat.name[2], "'Yes that would be fine'")
                MiniGame.questions()
                return None
        ...
        while True:
            ...
            while stat.answer == "0":
                if Stats.diary or Stats.confession == True:
                    ...
                    print(stat.name[5], "'My men are making their way towards Gelni, the two of you will join us there'")
                    print("Skye cups your face in her hand")
                    print(stat.name[5], "'I promise you sweetheart, this will work and we will all be okay'")
                    print(stat.name[0], "Arien which weapon would you prefer?\n[1]", battle.weapons[0], "\n[2]", battle.weapons[1], "\n[3]", battle.weapons[2], "\n[4]", battle.weapons[3])
                    battle.weapon = int(input())
                    battle.weapon -= 1
                    Battle.checkValid()
                    print("You pull up to the Riomian space station and begin embarking onto the space craft")
                    return None
                elif Stats.diary and Stats.confession == False:
                    print("You both get into the car")
                    print(stat.name[5], "'We will immediately depart for war, Arien which weapon do you prefer?\n[1]", battle.weapons[0], "\n[2]", battle.weapons[1], "\n[3]", battle.weapons[2], "\n[4]", battle.weapons[3])
                    battle.weapon = int(input())
                    battle.weapon -= 1
                    Battle.checkValid()
                    print("You pull up to the Riomian space station and begin embarking onto the space craft")
